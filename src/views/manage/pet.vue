    ...
    <div class="app-container">
        <pageTable @load-table-data="loadTableData" ref="dataTable" row-key="id" @selection-change="selectionChange">
            <template slot="function-button">
                ...
                <div class="search">
                    <div>宠物品种
                        <el-select v-model="searchData.breedId" filterable clearable placeholder="请选择">
                            <el-option v-for="item in breeds" :key="item.id" :label="item.name" :value="item.id">
                            </el-option>
                        </el-select>
                    </div>
                    <div>宠物昵称<el-input v-model="searchData.name" placeholder="输入昵称搜索" /></div>
            ...
            <template slot="tb-columns">
                ...
                <el-table-column label="品种" width="160" align="center">
                    <template slot-scope="scope">
                        {{ breeds[scope.row.breedId - 1].name }}
                    </template>
                </el-table-column>
...
import Edit from '@/components/Edit/PetEdit'
    ...
    methods: {
        ...
        del(ids, imgs) {
            ...
            this.$confirm("是否删除?", "提示", {
                ...
                postRequest("/pet/delete", { ids: ids }).then((response) => {
